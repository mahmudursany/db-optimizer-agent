...
class RemoteProjectScanner
{
    ...
    public function scan(string $targetUrl, string $token, array $paths): array
    {
        ...
        return $this->buildReport($baseUrl, $startedAt, $requests, $snapshots);
    }

    private function buildClient(string $token): PendingRequest
    {
        $request = Http::acceptJson()->timeout((int) config('db_optimizer.scanner.timeout_seconds', 20));
    ...
    private function pingAgent(PendingRequest $client, string $baseUrl): void
    {
        ...
        if (! $response->successful()) {
            throw new RuntimeException('Cannot connect to remote agent. Check URL/token and agent routes.');
        }
    }
