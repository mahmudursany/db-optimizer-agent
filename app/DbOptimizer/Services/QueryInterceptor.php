...
use App\DbOptimizer\Support\QueryOriginResolver;
...
class QueryInterceptor
{
    ...
    private function nPlusOneSignal(string $sql, int $repetition, array $origin): array
    {
        ...
        return [
            ...
            'reason' => $suspected
                ? 'Repeated single-row fetch pattern detected. Consider eager loading with with().'
                : null,
    ...
    private function cacheSignal(string $sql, int $repetition): array
    {
        ...
        return [
            ...
            'reason' => $isCandidate
                ? 'Query executed repeatedly with identical bindings. Consider Cache::remember().'
                : null,
            'repetition' => $repetition,
        ];
    }
    ...
    /**
     * Best-effort SQL interpolation for local diagnostics only.
     */
